                        ...
                        @forelse($degrees as $degree)
                            <tr class="bg-white dark:bg-gray-800">
                                ...
                                <td class="px-6 py-4 w-2/6  text-right">
                                    <a href="{{route('degree-program.edit',$degree->id)}}" class="text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-3 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 me-3">
                                        Edit
                                    </a>
                                    <form action="{{ route('degree-program.destroy', $degree->id) }}" method="POST" style="display:inline;" id="delete-form-{{$degree->id}}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDelete({{ $degree->id }})" class="text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-3 py-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">
                                            Delete
                                        </button>
                                    </form>
                                </td>
